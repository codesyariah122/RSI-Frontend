<template>
  <div class="berita__detail" :style="berita__list_style">
    <mdb-container>
      <!-- header -->
      <mdb-row class="row justify-content-start berita__detail-content">
        <mdb-col lg="12" xs="12" sm="12" class="mb-3">
          <mdb-btn @click="$router.go(-1)" color="danger" class="rounded-pill" :size="`${$device.isDesktop ? 'md' : 'sm'}`">
            <mdb-icon icon="angle-left" /> Kembali Ke Profile 
          </mdb-btn>
        </mdb-col>
        <mdb-col
        lg="12"
    ...
    <mdb-col lg="4" sm="12" xs="12">
        <mdb-btn :class="`btn my__btn-primary rounded-pill  ${
          $device.isMobile ? 'btn-block btn-sm' : 'btn-block'
        }`" @click="openModal">
        <mdb-icon icon="user-cog" :size="`${$device.isDesktop ? 'md' : 'sm'}`"/>
        Tambah Pekerjaan
      </mdb-btn>
    </mdb-col>
  </mdb-row>
